...
def _turbo_profile(**overrides: object) -> str:
    ...
    profile = {
        "userName": "testuser", "userId": "uid-1",
...
class TestBuildUrl:
    ...
    def test_absolute_url_passthrough(self) -> None:
        ...
        assert client._build_url(url) == url, (
            "Expected absolute URL returned unchanged"
        )
    ...
    def test_http_url_passthrough(self) -> None:
        ...
        assert client._build_url(url) == url, (
            "Expected http:// URL returned unchanged"
        )
...
class TestIsRedirectResponse:
    ...
    def test_false_for_normal_payload(self) -> None:
        ...
        client.login()
        resp = client._send_request("GET", SETTINGS_URL)
        assert client._is_redirect_response(resp) is False, (
            "Expected False for normal payload"
        )
    ...
    def test_false_for_non_json_body(self) -> None:
        ...
        resp = client._send_request("GET", SETTINGS_URL)
        assert client._is_redirect_response(resp) is False, (
            "Expected False when body is not JSON"
        )
...
class TestHandle202Redirect:
    ...
    def test_202_redirect_retries_after_root_data(self) -> None:
        ...
        assert resp.status_code == 200, (
            f"Expected 200 after retry, got {resp.status_code}"
        )
...
class TestUpdateUserSettings:
    ...
    def test_successful_update_posts_merged_payload(self) -> None:
        ...
        assert resp.status_code == 200, (
            f"Expected 200 from update, got {resp.status_code}"
        )
        # Verify POST was called
        post_calls = [c for c in responses.calls if c.request.method == "POST" and "index" in c.request.url]
        assert len(post_calls) == 1, (
            f"Expected 1 POST to update URL, got {len(post_calls)}"
        )
        # Verify payload contains weight=90
        body = post_calls[0].request.body
        assert "weight=90" in body, (
            f"Expected weight=90 in POST body, got {body}"
        )
    ...
    def test_update_merges_with_current_settings(self) -> None:
        ...
        assert "height=175" in body, (
            f"Expected current height preserved in POST, got {body}"
        )
        assert "weight=85" in body, (
            f"Expected updated weight=85 in POST, got {body}"
        )
    ...
    def test_update_includes_intent(self) -> None:
        ...
        post_calls = [c for c in responses.calls if c.request.method == "POST" and "index" in c.request.url]
        body = post_calls[0].request.body
        assert "intent=update-units" in body, (
            f"Expected intent=update-units in POST, got {body}"
        )
...
class TestConvenienceMethods:
    ...
    def test_get_training_zones_returns_model(self) -> None:
        ...
        zones_data = json.dumps([
            "userProfile", {"ftp": 200, "maxHeartRate": 190},
            "zones", {
        ...
        assert zones.ftp_watts == 200, (
            f"Expected ftp 200, got {zones.ftp_watts}"
        )
