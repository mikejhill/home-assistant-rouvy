...
class TestWeightSensor:
    ...
    def test_returns_weight_kg(self) -> None:
        ...
        assert _value_weight(p) == 85.5, f"Expected 85.5, got {_value_weight(p)}"

    def test_zero_weight_returns_none(self) -> None:
        p = _make_profile(weight_kg=0.0)
        assert _value_weight(p) is None, f"Expected None for zero weight, got {_value_weight(p)}"
...
class TestHeightSensor:
    ...
    def test_returns_height_cm(self) -> None:
        ...
        assert _value_height(p) == 180.0, f"Expected 180.0, got {_value_height(p)}"

    def test_zero_height_returns_none(self) -> None:
        p = _make_profile(height_cm=0.0)
        assert _value_height(p) is None, f"Expected None for zero height, got {_value_height(p)}"
...
class TestFtpSensor:
    ...
    def test_returns_ftp_watts(self) -> None:
        ...
        assert _value_ftp(p) == 250, f"Expected 250, got {_value_ftp(p)}"

    def test_zero_ftp_returns_none(self) -> None:
        p = _make_profile(ftp_watts=0)
        assert _value_ftp(p) is None, f"Expected None for zero FTP, got {_value_ftp(p)}"
...
class TestMaxHeartRateSensor:
    ...
    def test_returns_max_hr(self) -> None:
        ...
        assert _value_max_hr(p) == 195, f"Expected 195, got {_value_max_hr(p)}"

    def test_none_max_hr_returns_none(self) -> None:
        p = _make_profile(max_heart_rate=None)
        assert _value_max_hr(p) is None, f"Expected None, got {_value_max_hr(p)}"
...
class TestUnitsSensor:
    ...
    def test_returns_metric(self) -> None:
        ...
        assert _value_units(p) == "METRIC", f"Expected METRIC, got {_value_units(p)}"

    def test_returns_imperial(self) -> None:
        p = _make_profile(units="IMPERIAL")
        assert _value_units(p) == "IMPERIAL", f"Expected IMPERIAL, got {_value_units(p)}"
...
class TestNameSensor:
    ...
    def test_full_name(self) -> None:
        ...
        assert _value_name(p) == "John Doe", f"Expected 'John Doe', got {_value_name(p)}"

    def test_first_name_only(self) -> None:
        p = _make_profile(first_name="John", last_name="")
        assert _value_name(p) == "John", f"Expected 'John', got {_value_name(p)}"

    def test_last_name_only(self) -> None:
        p = _make_profile(first_name="", last_name="Doe")
        assert _value_name(p) == "Doe", f"Expected 'Doe', got {_value_name(p)}"

    def test_fallback_to_username(self) -> None:
        p = _make_profile(first_name="", last_name="", username="jdoe")
        assert _value_name(p) == "jdoe", f"Expected 'jdoe' fallback, got {_value_name(p)}"
