...
class TestUserProfileDataclass:
    ...
    def test_all_defaults(self) -> None:
        ...
        assert profile.gender is None, (
            f"Expected None default for gender, got {profile.gender}")
        assert profile.birth_date is None, (
            f"Expected None default for birth_date, got {profile.birth_date}")
        assert profile.country is None, (
            f"Expected None default for country, got {profile.country}")
        ...
        assert profile.user_id is None, (
            f"Expected None default for user_id, got {profile.user_id}")

    def test_with_all_optional_fields(self) -> None:
        profile = UserProfile(
            email="full@test.com", username="full",
            first_name="Full", last_name="User",
            weight_kg=90.0, height_cm=185.0,
            units="IMPERIAL", ftp_watts=300, ftp_source="ESTIMATED",
            max_heart_rate=195, gender="MALE",
            birth_date=date(1990, 6, 15),
            country="GB", timezone="Europe/London",
            account_privacy="PRIVATE", user_id="uid-999",
        )
        assert profile.max_heart_rate == 195, (
            f"Expected max_heart_rate 195, got {profile.max_heart_rate}")
        assert profile.gender == "MALE", (
            f"Expected gender MALE, got {profile.gender}")
        assert profile.birth_date == date(1990, 6, 15), (
            f"Expected birth date 1990-06-15, got {profile.birth_date}")
        assert profile.country == "GB", (
            f"Expected country GB, got {profile.country}")

    def test_frozen_immutable(self) -> None:
        profile = UserProfile(
            email="e", username="u", first_name="f", last_name="l",
            weight_kg=0, height_cm=0, units="METRIC", ftp_watts=0, ftp_source="",
        )
    ...
    def test_zero_weight_and_height(self) -> None:
        profile = UserProfile(
            ...
            weight_kg=0.0, height_cm=0.0, units="METRIC", ftp_watts=0, ftp_source="",
        )
        assert profile.weight_kg == 0.0, (
            f"Expected zero weight, got {profile.weight_kg}")
        assert profile.height_cm == 0.0, (
            f"Expected zero height, got {profile.height_cm}")
...
class TestTrainingZonesDataclass:
    ...
    def test_with_zone_values(self) -> None:
        zones = TrainingZones(
            ftp_watts=250, max_heart_rate=195,
...
class TestConnectedAppDataclass:
    ...
    def test_required_fields_only(self) -> None:
        ...
        assert app.provider_id == "strava", (
            f"Expected provider_id strava, got {app.provider_id}")
        assert app.upload_mode is None, (
            f"Expected None upload_mode, got {app.upload_mode}")
        assert app.permissions == [], (
            f"Expected empty permissions, got {app.permissions}")

    def test_with_all_fields(self) -> None:
        app = ConnectedApp(
            provider_id="garmin", name="Garmin Connect", status="active",
            upload_mode="auto", description="Sync activities",
            logo_path="/logos/garmin.png",
            permissions=["ACTIVITY_EXPORT", "PROFILE_READ"],
        )
        assert app.upload_mode == "auto", (
            f"Expected upload_mode auto, got {app.upload_mode}")
        assert len(app.permissions) == 2, (
            f"Expected 2 permissions, got {len(app.permissions)}")
...
class TestActivityDataclass:
    ...
    def test_required_fields_only(self) -> None:
        act = Activity(
            activity_id="act-1", title="My Ride",
            training_type="ROUTE_TIME_TRIAL",
            distance_m=25000.0, moving_time_seconds=3600,
            elevation_m=400.0,
        )
        assert act.activity_id == "act-1", (
            f"Expected activity_id act-1, got {act.activity_id}")
        assert act.start_utc is None, (
            f"Expected None default for start_utc, got {act.start_utc}")
        assert act.intensity_factor is None, (
            f"Expected None default for intensity_factor, got {act.intensity_factor}")

    def test_with_all_optional_fields(self) -> None:
        act = Activity(
            activity_id="act-2", title="Evening Spin", training_type="WORKOUT",
            distance_m=15000.0, moving_time_seconds=1800, elevation_m=200.0,
            intensity_factor=0.95, start_utc="2024-01-15T08:00:00Z",
        )
    ...
    def test_frozen_immutable(self) -> None:
        act = Activity(
            activity_id="x", title="x", training_type="",
    ...
    def test_zero_distance_and_time(self) -> None:
        act = Activity(
            activity_id="z", title="Zero", training_type="",
            distance_m=0.0, moving_time_seconds=0, elevation_m=0.0,
        )
        assert act.distance_m == 0.0, (
            f"Expected 0.0 distance, got {act.distance_m}")
...
class TestActivitySummaryDataclass:
    ...
    def test_with_activities(self) -> None:
        act = Activity(
            activity_id="a", title="t", training_type="",
            distance_m=100, moving_time_seconds=60, elevation_m=0,
        )
